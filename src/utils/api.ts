...
api.interceptors.request.use(
  (config) => {
    ...
    const isPublicRoute = url.includes('/api/public') || (method === 'get' && url.includes('/api/comments/perfume/'))
  ...
  (error) => {
    ...
    if (!error.response) {
      if (isBackendConnectionError(error)) {
        error.message = 'Backend server is not running. Please start your backend server.'
      } else {
...
export const publicAPI = {
  ...
  getPerfumeComments: (perfumeId: string) => api.get(`/api/comments/perfume/${perfumeId}`),
}
...
export const protectedAPI = {
  ...
  updatePerfume: (id: string, data: any) => api.put(`/api/perfumes/${id}`, data),
  ...
  // Perfume Tags
  getPerfumeTags: (perfumeId: string) => api.get(`/api/perfume-tags/perfume/${perfumeId}`),
  getUserTaggedPerfumes: (params?: { tag?: string; favorite?: boolean; wishlist?: boolean }) => 
    api.get('/api/perfume-tags/user/tagged', { params }),
  getUserTagStats: () => api.get('/api/perfume-tags/user/stats'),
  addPerfumeTags: (perfumeId: string, data: any) => api.post(`/api/perfume-tags/perfume/${perfumeId}`, data),
  ...
  toggleFavorite: (perfumeId: string) => api.patch(`/api/perfume-tags/perfume/${perfumeId}/favorite`),
  toggleWishlist: (perfumeId: string) => api.patch(`/api/perfume-tags/perfume/${perfumeId}/wishlist`),
  ...
  // Comments
  getPerfumeComments: (perfumeId: string) => api.get(`/api/comments/perfume/${perfumeId}`),
  addComment: (perfumeId: string, data: { content: string; rating?: number }) => api.post(`/api/comments/perfume/${perfumeId}`, data),
  getAllComments: () => api.get('/api/comments/all'),
  updateComment: (commentId: string, data: any) => api.put(`/api/comments/${commentId}`, data),
  deleteComment: (commentId: string) => api.delete(`/api/comments/${commentId}`),
