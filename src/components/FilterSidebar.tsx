...
export default function FilterSidebar({ onFiltersChange }: FilterSidebarProps) {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const activeBrands = response.data.filter((brand: Brand) => !brand.isDeleted)
  ...
  const handleFilterChange = (filterType: keyof typeof selectedFilters, value: string | number) => {
              ...
              brands.map((brand) => (
                <label key={brand._id} className="flex items-center">
                  <input
                    ...
                    className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
                  />
                  <span className="ml-2 text-sm text-gray-700">{brand.brandName}</span>
                </label>
              ))
            )}
          </div>
