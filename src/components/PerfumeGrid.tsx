...
export default function PerfumeGrid({
  ...
  useEffect(() => {
    ...
    // Gender filter (normalize casing between filter values and perfume.targetAudience)
    if (filters?.gender && filters.gender.length > 0) {
      filtered = filtered.filter((perfume) =>
        filters.gender!.some(
          (g) =>
            g.toLowerCase() === String(perfume.targetAudience).toLowerCase()
        )
      );
    }
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters, searchQuery, sortBy, filteredPerfumes.length]);
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(sortedPerfumes.length / itemsPerPage)
  );
  const pageItems = sortedPerfumes.slice(
